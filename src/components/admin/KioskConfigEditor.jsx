...
export default function KioskConfigEditor({ kiosk, onBack, onSaveSuccess }) {
  ...
  const [formData, setFormData] = useState({
    name: kiosk?.name || '',
    license_key: kiosk?.license_key || kiosk?.licenseKey || '',
    pin: kiosk?.pin || '1111',
      ...
      const res = await fetch('/api/admin/kiosks', {
        ...
        body: JSON.stringify(
          isNew
            ...
            : {
                action: 'update_kiosk',
                license_key: kiosk?.license_key || kiosk?.licenseKey || formData.license_key,
                updates: {
                  ...formData,
                  license_key: kiosk?.license_key || kiosk?.licenseKey || formData.license_key,
                  kiosk_mode: formData.kiosk_mode || (formData.is_event_mode ? 'event' : 'regular'),
                  is_event_mode: formData.kiosk_mode === 'event' || Boolean(formData.is_event_mode),
                },
              }
        ),
      });
